...
# # 2) save dataset
# save_dataset("sokoban_3_3_dataset", all_traces, obs_occurences, unique_obs_img, unique_transitions)
...
def export_dataset(action_type="full",
    ...
    if not os.path.exists("Sokoban_3_3_FullDFA"+init_st+"-"+goal_st):
        os.makedirs("Sokoban_3_3_FullDFA"+init_st+"-"+goal_st) 
    exp_folder = "Sokoban_3_3_FullDFA"+init_st+"-"+goal_st
    global counntteerr
    ...
    if remove_some_trans:
        loaded_removable_trans = load_dataset("/workspace/pddlgym-tests/pddlgym/sokoban3_3_missing_edges/deleted_edges.p")
        transis_to_remove = loaded_removable_trans["stuff"]

        # loaded_removable_edge_goal = load_dataset("/workspace/pddlgym-tests/pddlgym/sokoban6_6_missing_edges/goal_state.p")
        # edge_removed_goal = loaded_removable_edge_goal["stuff"]
        # goal_st = edge_removed_goal

        # loaded_removable_edge_init = load_dataset("/workspace/pddlgym-tests/pddlgym/sokoban6_6_missing_edges/init_state.p")
    ...
    if not os.path.exists("sokoban-3-3_FullDFA-no-noise-"+init_st+"-"+goal_st):
        os.makedirs("sokoban-3-3_FullDFA-no-noise-"+init_st+"-"+goal_st) 
    exp_folder_ = "sokoban-3-3_FullDFA-no-noise-"+init_st+"-"+goal_st
    ...
    if add_noisy_trans:
        ...
        if not os.path.exists("sokoban_3_3_perc"+str(perc)+"_#v"+str(number_noisy_versions)+"_std"+str(std)):
            os.makedirs("sokoban_3_3_perc"+str(perc)+"_#v"+str(number_noisy_versions)+"_std"+str(std)) 
        exp_folder = "sokoban_3_3_perc"+str(perc)+"_#v"+str(number_noisy_versions)+"_std"+str(std)

    elif remove_a_trans:
        # creating init/goal dir for the experiment
        if not os.path.exists("sokoban_3_3_RemovedTrans-"+trans_to_remove_list[0]+"-"+trans_to_remove_list[1]):
            os.makedirs("sokoban_3_3_RemovedTrans-"+trans_to_remove_list[0]+"-"+trans_to_remove_list[1]) 
        exp_folder = "sokoban_3_3_RemovedTrans-"+trans_to_remove_list[0]+"-"+trans_to_remove_list[1]

    elif remove_some_trans:
        if not os.path.exists("sokoban_3_3_missing_edges"):
            os.makedirs("sokoban_3_3_missing_edges") 
        exp_folder = "sokoban_3_3_missing_edges"


    # mean and std for the noise
    mean = 0
    # sokoban_3_3_dataset
    loaded_dataset = load_dataset("/workspace/pddlgym-tests/pddlgym/sokoban_3_3_dataset/data.p")
    ...
    for iii, trace in enumerate(loaded_traces):
        ...
        for trtrtr, transitio in enumerate(trace):
            ...
            # gaussian_noise_0 = np.random.normal(0, 0.2, transitio[0][0].shape)
            # noisy1_ = transitio[0][0] + gaussian_noise_0
            # plt.imsave("SOKOUUU9.png", reduce_resolution(transitio[0][0]))
            # plt.close()
            # exit()

            # gaussian_noise_1 = np.random.normal(0, 0.2, transitio[0][1].shape)
            # noisy2_ = transitio[0][1] + gaussian_noise_1
            # plt.imsave("soko_transi_noise_1_1.png", np.clip(noisy2_, 0, 1))
            # plt.close()
    ...
    print(len(all_actions_unique)) # 182
    #exit()
